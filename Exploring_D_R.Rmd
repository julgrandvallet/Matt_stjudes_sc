...
lowexp_filt_stjudes <- read_csv("tasks_results/lowexpressing_filteredout/lowexpr_stjudes_LH_NH_zscore2.5_ln2pfpkm1.csv") %>% 
  column_to_rownames("...1")

genes_to_keep <- rownames(lowexp_filt_stjudes)
...
perform_t_test <- function(gene_expression, diagnosis_samples, relapse_samples) {
  t_test_result <- t.test(
    gene_expression[relapse_samples], 
    gene_expression[diagnosis_samples], 
    alternative = "greater",  # One-tailed test: relapse > diagnosis
    paired = FALSE            # Assuming samples are NOT paired
  )
  return(t_test_result$p.value)
}

# Get column indices for LH Diagnosis and LH Relapse samples
lh_diagnosis_cols <- grep("LH.*-D", colnames(filtered_bulk_matrix_subset))
lh_relapse_cols <- grep("LH.*-R", colnames(filtered_bulk_matrix_subset))

# Perform t-test for each gene and store p-values
p_values <- apply(filtered_bulk_matrix_subset, 1, function(gene_expression) {
  perform_t_test(gene_expression, lh_diagnosis_cols, lh_relapse_cols)
})

# Create a comparison data frame
comparison_df <- data.frame(
  Gene = rownames(filtered_bulk_matrix_subset),
  Avg_LH_Diagnosis = rowMeans(filtered_bulk_matrix_subset[, lh_diagnosis_cols], na.rm = TRUE),
  Avg_LH_Relapse = rowMeans(filtered_bulk_matrix_subset[, lh_relapse_cols], na.rm = TRUE),
  p_value = p_values,
  stringsAsFactors = FALSE
)

# Rank genes by raw (unadjusted) p-value in ascending order (smallest p-value = most significant)
comparison_df <- comparison_df %>% arrange(p_value)

# Apply Benjamini-Hochberg correction for multiple testing
comparison_df$adjusted_p_value <- p.adjust(comparison_df$p_value, method = "BH")

# View top-ranked genes by raw p-value
head(comparison_df)

# Optionally, save results to a CSV file
write.csv(comparison_df, "LH_gene_ranking_by_ttest.csv", row.names = FALSE)
...
write.csv(lowexp_filt_heatmap_data, "tasks_results/lowexpressing_filteredout/lowexpr_stjudes_LH_NH_zscore2.5_ln2pfpkm1.csv", row.names = T)
...
```{r}
Gene_List_for_Julian <- read_excel("Gene List for Julian.xlsx")

Gene_List_for_Julian
```
...
gene_list_HF <- Gene_List_for_Julian %>% 
    filter(type == "HF") %>% 
    pull(Gene)

gene_list_LF <- Gene_List_for_Julian %>% 
    filter(type == "LF") %>% 
    pull(Gene)


# Perform GO enrichment analysis
go_enrich_HF <- enrichGO(gene = gene_list_HF, OrgDb = org.Hs.eg.db, keyType = "SYMBOL", ont = "BP", pAdjustMethod = "BH", qvalueCutoff = 0.05)
go_enrich_LF <- enrichGO(gene = gene_list_LF, OrgDb = org.Hs.eg.db, keyType = "SYMBOL", ont = "BP", pAdjustMethod = "BH", qvalueCutoff = 0.05)
